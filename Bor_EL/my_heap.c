...
heap_t create_heap(int n,int *array_P,comp_func_t *gt, comp_func_t * lt)
{
	...
	heap->n_total=n;
	heap->n_elements=0;
	heap->array=malloc(sizeof(KEY)*n);	
	heap->gt=gt;
	heap->lt=lt;
	return (heap);
}
...
int heap_extract_min(heap_t heap, KEY * key)
{
	KEY * A,min;
...
int heap_insert(heap_t heap, KEY key)
{
...
int heap_decrease_key(heap_t heap, int position, KEY new_key)
{
	KEY * A;
	int i,*array_P;
